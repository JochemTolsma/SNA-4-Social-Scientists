...
cv_total <- cv_tot  %>%
  ...
  ungroup()
#because group_by() returns a grouped tibble (a tibble specific class with a group attribute), it's necessary to close the pipe-chain with ungroup() to avoid an error with the mice function, which takes a data frame or matrix as input (and can't handle tibbles). Note that without ungrouping, the output looks the same, but the data structure is different.
```
...
#thus, the following two commands return the same output.
mutate_at(c("men2", "educ3b", "region"), as.factor)
...
#the pluck() function from the Purrr package (part of the tidyverse) is the user-friendly equivalent of `[[` in base R: `[[`("predictorMatrix"), which
#enables you to extract a single element from the list we just created using unclass().
#next, because the mutate function (and its variations) are not build for lists, we need to coerce it a data frame (or Tibble)
#lastly, to 'remove' the columns id id2 and wave as predictors, we recode all their 1s to 0s using mutate_at() and recode().
