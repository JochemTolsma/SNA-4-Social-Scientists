...
age_region <- group_by(cv_tot, region)

#step 2. use mutate to create a append the mean age by region to the original data frame (don't forget to remove NA values).
...
model1 <- cv_total %>%
  ...
  summary()
#the across fucntion from the Dplyr package is similar to the apply function (if MARGIN = 2) from base R as it performs the same operation on multiple columns (or, if you have groups, for each combination of columns and groups).

#note on lm: lm is not a 'pipe friendly' function because the data source argument is the function's second argument, while the pipe operator uses the first argument that follows the %>%.
#you can overwrite this default behavior by specifying the parameter names, so the pipe function knows where (not) to look.
#in this case, lm's first argument is a formula, which needs explicit mention to avoid an error.
```

```{r, results="hold"}
#step 1. define all missings (and save a version of the dataset without replacement of the missings for later.
cv_orig <- cv_total <- cv_total %>%
  mutate(health = na_if(health, -2),
         healthn = as.numeric(health) - 4)

#if you don't know the numeric value of "Weigert" (-2) you can extract it from the object's attributes using attr().
cv_orig <- cv_total <- cv_total %>%
  mutate(health = na_if(health, attr(health, "labels")["Weigert"]), 
         healthn = health - 4)
```
...
```{r, listwise}
model2 <- cv_total %>%
  mutate(healthn = as.numeric(healthn),
         across(c(educ3, region), as.factor)) %>%
  lm(formula = healthn ~ men + age + educ3 + region) %>%
  summary()
```
...
# Besides the `ifelse()` command, you could also use `replace_na` from the Tidyr package (part of the tidyverse), which, you might have guessed from the name, enables you to replace NAs with specified values.
...
```{r replace, results="hold"}
#example of using ifelse() with mutate.
cv_total <- cv_total %>%
  mutate(men2 = ifelse(is.na(men), 2, men))

#with the first argument of the mutate function, you avoid repeating as.numeric() later on.
cv_total <- cv_total %>%
...
#note: the following two commands are equivalents:
mutate(across(everything(), as.numeric))
...
#note: for model2 we used mutate(across()) which is similar to mutate_at(). The latter is shorter, but less flexibel as the transformation affects every variable.
#or, in other words, the following two commands return the same output.
mutate_at(c("men2", "educ3b", "region"), as.factor)
...
```{r mice, results="hold", error=TRUE }
#let's start with the original dataset 'cv_orig' (created in the first section).
#don't forget to remove healthn (last column) because it perfectly correlates with health which will cause an error. 
pred <- cv_orig  %>%
  select(-last_col()) %>%
  mutate_all(as.numeric)  %>%
  ...
  as.data.frame() %>%
  mutate_at(funs({0}), .vars =  c("id", "id2", "age")) %>%
  as.matrix()
#note: the object returned from the mice function is of class 'mids,' which dplyr's functions cannot handle.
#the unclass function from base R solve this issue as it removes the class attribute and, in this case, returns a list that dplyr can handle.
#next, the pluck function from the Purrr package (part of the tidyverse) is the user-friendly equivalent of `[[` in base R: `[[`("predictorMatrix"), which
#enables you to extract a single element from the list we just created using unclass().
#lastly, because the mutate function (and its variations) are not build for lists, we need to coerce it a data frame (or Tibble)

#impute missings and fit model on imputed dataset
model_imp <- cv_total %>%
  mutate_at(c("men", "educ3"), as.factor) %>%
