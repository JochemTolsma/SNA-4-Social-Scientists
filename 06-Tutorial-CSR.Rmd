...
#step 2. use "summarise" (also from the dplyr package) to create a new data frame containing the mean age per region. Note: summarise() automatically creates a new data frame.
age_region <- summarise(by_region, mean = mean(age))
...
age_region <- group_by(cv_tot, region) %>% summarise(mean = mean(age))
```
...
model1 <- cv_total %>%
  ...
  summary()
#note: adding lm directly to the pipeline without specifying the parameter's name (formula) will raise an error.
```

```{r, results="hold"}
#recode health and save a version of the dataset without replacement of the missings for later.
...
#note: for model2 we used mutate(across()) which is similar to mutate_at(), which is shorter but less flexibel as the transformation affects every variable.
#in sum, the two following commands return the same output.
mutate_at(c("men2", "educ3b", "region"), as.factor)
...
pred <- cv_orig  %>%
  ...
  as.matrix()
#note: the pluck function from the Purrr package (part of the tidyverse) is the user-friendly equivalent of `[[` in base R: `[[`("predictorMatrix")
