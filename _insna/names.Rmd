...
Furthermore, dealing with files like this requires a little bit of 'regular expression magic'. Regular expressions help you to transform text (string) data, by calling either to specific strings (e.g. the word "dog"), or for certain generalizable string patterns (e.g. a string consisting of exactly three lowercase letters). Everything I have learned about string manipulation basically stems from [this webpage](https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html). Especially when using web-scraped data, you will encounter raw text data pretty regularly, so knowing about regular expressions will probably come in handy. 
...
There is also a handful of exceptions, which we need to deal with in some way. For the second and third names in the data ("Marcel van Assen", "Weverthon Barbosa Machado"), we see that the full name consists of more than 2 words. 
...
I start by extracting the last names and nobiliary particles. 

I tend to first extract multiple-word nobiliary particles, and then the single-word nobiliary particles because otherwise, you risk only extracting the first word of a nobiliary particle consisting of multiple words (e.g. you extract only "van", while the complete nobiliary particle is "van de"). The majority of these nobiliary particles are highly specific to Dutch names to match to the database from Dutch universities. 


After I have stored the nobiliary particle in a separate object, I use two strategies for extracting the rest of the last name:

* 1: If there IS a nobiliary particle present: we extract everything after this, because we can be pretty sure that no first names will follow the nobiliary particle
* 2: If there is NO nobiliary particle present: we extract just the last word in the string to be safe
...
# lastname: everything after nobiliary particle if there is one, otherwise we simply take the last word in the string
...
Due to the standard format of the name object, we can simply take out the first word of the name string. Please note that some people only have initials instead of a first name, but given we don't have much more information, there is not much we can do about that at this point in time. 
