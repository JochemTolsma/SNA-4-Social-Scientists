...
* To what extent does the educational level of our confidants influence our political opinions?  

* To what extent does the impact of the educational level of our confidants on our political opinion depend on:  
  - ego characteristics?  
  - characteristics of our the Core Discussion Network as a whole?  
  - charactersitics of our confidants?  

Naturally, we need to be aware that our own educational-level (and political opinion) will influence with whom we discuss important matters. Thus, we need to take into account selection effects.  
...
data_clean <- data_long %>%
  select(c("nomem_encr", "survey_wave", "euthanasia", "income_diff","immigrants", "eu_integration",
  ...
  arrange(ego_id, survey_wave)


data_clean_wide <- reshape(data_clean, direction = "wide", idvar = "ego_id", timevar = "survey_wave", v.names=c("euthanasia", "income_diff","immigrants", "eu_integration","educ_alter1", "gender_alter1", "age_alter1",
                                                                                                                "educ_alter2", "gender_alter2", "age_alter2",
                                                                                                                "educ_alter3", "gender_alter3", "age_alter3",
...
[partner_dataprepped.Rdata](addfiles\liss_cdn.Rdata)\
...
-   nomen_encr: ego id  
-   eu: opinion of ego on eu_integration: 0 = eu integration has gone too far / 4 = eu integration should go further
-   educego: educuational level of ego = highest completed education in years (4-16.5)  
-   educalter: educuational level of alter = highest completed education in years (4-16.5)
