...
webElem <- remdr$findElement(using = "class", value = "lst")
...
recht_staff <- read_html("https://www.ru.nl/rechten/over-de-faculteit/vaksecties/rechtssociologie/medewerkers-0/")
 ...
 emailjs <- recht_staff %>% html_nodes('body') %>% html_nodes('script') %>% html_text()
 library(V8)
ct <- v8()
  ct$eval(gsub('document.write','',emailjs[1])) 
...
remDr$navigate("www.jochemtolsma.nl")
...
### examples
```{r}
jo <- "J Tolsma, Radboud University" #Please change this to your own name!!!!
df <- data.frame(firstname=c("Jochem", "Anne", "Bas"))
...
network2001_2010 <- matrix(NA, nrow=nrow(soc_df), ncol=nrow(soc_df))
...
for (ego in 1: nrow(soc_df)) {
  ...
  for (alter in 1:nrow(soc_df)){
    ...
    network2001_2010[ego,alter] <- sum(str_detect(pubs_sel2$author, name_alter)) > 1 #did alter publish with ego
  }
}
