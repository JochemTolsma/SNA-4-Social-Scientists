...
```{r ud, echo=FALSE, , eval=TRUE, fig.cap="undirected dyad configurations", out.width = '50%'}
...
```{r dd, echo=FALSE, , eval=TRUE, fig.cap="undirected dyad configurations", out.width = '100%'}
...
```{r mud, echo=FALSE, , eval=TRUE, fig.cap="multiplex, undirected dyad configurations", out.width = '100%'}
...
```{r mdd, echo=FALSE, , eval=TRUE, fig.cap="multiplex, undirected dyad configurations", out.width = '100%'}
...
Can you make a guess about the resulting degree of eductional homogamy?? Press `r colorize("Update!", "red")` to see if you were correct. 
...
```{r pi, echo=FALSE, fig.cap="Positive influence"}
library(ggplot2)
time <- rep(c(1:10),2)
...
```{r ni, echo=FALSE, fig.cap="Negative influence"}
library(ggplot2)
time <- rep(c(1:10),2)
...
```{r pfi, echo=FALSE, fig.cap="Positive feedback influence"}
library(ggplot2)
time <- rep(c(1:10),2)
couple <- rep(c("man", "woman"), each=10)
opinion1 <- 4 + .5*c(1:10)
...
```{r shocks, echo=FALSE , fig.cap="Opinion development over time: parallel trend with shocks."}
...
p1 <- ggplot(data, aes(x=x) ) +
  # Top
  geom_density( aes(x = var1, y = ..density..), fill="#69b3a2" , alpha=.4, bw=0.3) +
  geom_label( aes(x=0, y=-0.05, label="individuals"), color="#69b3a2") +
  theme_ipsum() +
...
```{r, echo=FALSE, fig.cap="Distribution of opinions within population"}
p1
```
...
It turns out the polarization score of the above society is `fpol(opinions, 20)` `r fpol(pol1, 20)`. 
...
```{r, echo=FALSE}
set.seed(98712)
pol2 <- data2$mean <- rowMeans(data2)

#selective marriage: couples
p2 <- ggplot(data2, aes(x=x) ) +
  # Top
  geom_density( aes(x = mean, y = ..density..), fill="#404080" , alpha=.4, bw=0.3) +
  geom_label( aes(x=0, y=-0.05, label="couples"), color="#404080") +
  theme_ipsum() +
  xlab("some political opinion") + 
  xlim(-10,10) + 
  ylim(-0.05,.3)
...
```{r , echo=FALSE, fig.cap="Distribution of opinions within couples after selective marriage"}
p2
```

Well, the preference for a partner with similar political opinions was definitely not strong enough to substantially change the opinion distribution.[^sim] The level of polarization (between couples) is: `r fpol(pol2, 20)` 
...
set.seed(98712)
#common influence
data2$mean2 <- ifelse(data2$mean > mean(data2$mean), data2$mean + abs(rnorm(1,2,.3)), data2$mean - abs(rnorm(1,2,.3)))

data2$mean2[data2$mean2 > 10] <- 10
data2$mean2[data2$mean2 < -10] <- -10
...
p3 <- ggplot(data2, aes(x=x) ) +
  # Top
  geom_density( aes(x = mean2, y = ..density..), fill="#404080" , alpha=.4, bw=0.3) +
  geom_label( aes(x=0, y=-0.05, label="couples"), color="#404080") +
  theme_ipsum() +
  xlab("some political opinion") + 
  xlim(-10,10)  + 
...
```{r, echo=FALSE, fig.cap="Distribution of opinions within couples after selective marriage and shared life events"}
p3
```

We now clearly observe emerging polarization between couples. But this is not yet reflected in our measure of polarization: `r fpol(pol3, 20)`. Probably because the opposing opinions are not close to the extremes (set at -10 and 10). Take home message: don't always 'trust' your numbers but really 'look' at your data as well.   
...
data2$X1b[data2$X1b > 10] <- 10
data2$X1b[data2$X1b < -10] <- -10
data2$X2b[data2$X2b > 10] <- 10
data2$X2b[data2$X2b < -10] <- -10

pol4 <- var1 <- c(data2$X1b,data2$X2b)
data <- data.frame(var1)
...
Here the level of polarization is: `r fpol(pol4, 20)`.

```{r, echo=FALSE, fig.cap="Distribution of opinions within population after selective marriage and shared life events"}
p4
```
...
#within couple positive influence
data2$X1c <- data2$X1b + .9*(data2$mean2 - data2$X1b)
data2$X2c <- data2$X2b + .9*(data2$mean2 - data2$X2b)

pol5 <- var1 <- c(data2$X1c,data2$X2c)
data <- data.frame(var1)
...
```{r, echo=FALSE, fig.cap="Distribution of opinions within population after selective marriage, shared life events and positive influence"}
p5
```


We already observe clear 'opinion groups'. The level of polarization is now: `r fpol(pol5, 20)`. These groups may start to further move apart due to two mechanisms: (1) shared context at the dyad-level; (2) by positive feedback influence.

```{r, echo=FALSE}

set.seed(98712)

#common influence / which has same result as positive feedback mechanism

data2$mean3 <- ifelse(data2$mean2 > 4, data2$mean2 + abs(rnorm(1,3,.3)), data2$mean2 - abs(rnorm(1,3,.3)))

data2$mean3[data2$mean3 > 10] <- 10
data2$mean3[data2$mean3 < -10] <- -10


data2$X1d <- data2$X1c + (data2$mean3 - data2$mean2)
data2$X2d <- data2$X2c + (data2$mean3 - data2$mean2)

data2$X1d[data2$X1d > 10] <- 10
data2$X1d[data2$X1d < -10] <- -10
data2$X2d[data2$X2d > 10] <- 10
data2$X2d[data2$X2d < -10] <- -10


pol6 <- var1 <- c(data2$X1d,data2$X2d)
data <- data.frame(var1)
#try different kernels
p6 <- ggplot(data, aes(x=var1) ) +
  # Top
  geom_density( aes(x = var1, y = ..density..), fill="#69b3a2" , alpha=.4, bw=0.3) +
  #geom_histogram() + 
  geom_label( aes(x=0, y=-0.05, label="individuals"), color="#69b3a2") +
...
# https://stackoverflow.com/questions/25510331/join-two-ggplot2-histograms-and-density-plots
# ggplot(data, aes(x=var1) ) +, kernel="triangular"
#   geom_histogram()
...
```{r, echo=FALSE, fig.cap="Distribution of opinions within population after selective marriage, shared life events, positive influence and feedback influence"}
p6
```
Now, the society is clearly starting to polarize. Yes! Suprisingly, although the score on our polarization measure is starting to increase (to: `r fpol(pol6, 20)`), the degree of polarization is still far off from 1. 

```{r, echo=FALSE}
pol7 <- ifelse(pol6 < -5, -5, pol6)
```

Suppose instead that the theoretical range of the opinion would have been $[-5,10]$. Then our polarization score would have been a massive: `r fpol(pol7, 15)`. 
